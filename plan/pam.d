...
DESCRIPTION
       When a PAM aware(注意到) privilege granting application(应用权限授予) is started, it
       activates its attachment(附属品) to the PAM-API. This activation(激活) performs(执行) a
       number of tasks, the most important being(本质/是) the reading of the
       configuration file(s): /etc/pam.conf. Alternatively(二选一), this may be the
       contents of the /etc/pam.d/ directory. The presence(存在) of this directory
       will cause Linux-PAM to ignore /etc/pam.conf.

       These files list the PAMs that will do the authentication tasks
       required(需要) by this service, and the appropriate(适当) behavior(行为) of the PAM-API
       in the event that individual(个人/单个) PAMs fail.

       The syntax of the /etc/pam.conf configuration file is as follows. The
       file is made up of a list of rules, each rule is typically(通常) placed(放置) on a
       single line, but may be extended(扩大的) with an escaped(逃脱) end of line: ‘\<LF>´.
       Comments are preceded(之前) with ‘#´ marks and extend(补充) to the next end of
       line.

       The format of each rule is a space separated(分开的) collection(收集) of tokens(符号), the
       first three being case-insensitive(大小写不敏感):


    service type control module-path module-arguments

       The syntax of files contained(包含) in the /etc/pam.d/ directory, are
       identical(相同) except(除) for the absence(缺席/没有) of any service field(领域/字段). In this case(在这种情况下),
       the service is the name of the file in the /etc/pam.d/ directory. This
       filename must be in lower(降低) case(小写).

       An important feature of PAM, is that a number of rules may be stacked(v.堆放)
       to combine(联合) the services of a number of PAMs for a given authentication
       task.

       The service is typically(代表性的) the familiar(常见的) name of the corresponding(相当的)
       application: login and su are good examples. The service-name, other,
       is reserved(保留) for giving default rules. Only lines that mention(提及) the
       current service (or in the absence of such, the other entries) will be
       associated(联合的) with the given service-application.
...
Linux-PAM Manual          03/02/2009               PAM.CONF(5)
